...
long long largestProduct(int n) {
    // 檢查陣列是否為空或窗口大小是否合理
    if (thousandDigits.empty() || n > thousandDigits.size()) return -1; 
    
    long long currentProduct;
    long long maxProduct = -1;
    int left = 0;
    int right = n - 1; // 設定窗口大小，設 n 為 4，窗口為 index 0 ～ 3

    while (right < thousandDigits.size()) {
        currentProduct = 1; // 先重設目前為 1
        for (int i = left; i <= right; ++i) { // 確保只計算窗口大小範圍
            currentProduct *= thousandDigits[i]; // 將窗口範圍計算
        }
        if (currentProduct > maxProduct) { // 如果比目前的 maxProduct 還大，就更新
...
int main() {
    std::cout << largestProduct(13) << std::endl; // 23514624000
